...
class MainProcessScreen extends StatefulWidget {
  final int mainkey;
  // const MainProcessScreen({super.key ,required this.mainkey});
    MainProcessScreen({
    Key? key,
...
class _MainProcessScreenState extends State<MainProcessScreen> {
  DateTime selectedDate = DateTime.now();
  // ==================== الإضافة الأولى: متغير للتاريخ الجديد ====================
  ...
  Future<List<Map<String, dynamic>>> fetchGroupedProcesses() async {
    ...
    // ==================== التعديل: استخدام التاريخ الجديد في الاستعلام ====================
    final response = await supabase.rpc('fetch_process_data', params: {
      'farm_code_param': '$New_user_area%',
      'date_from_param': selectedDate.toIso8601String(),
      'date_to_param': selectedToDate.toIso8601String(), // استخدام المتغير الجديد هنا
      'user_id_param': userIdInt
    });
    // ===================================================================================
  ...
  // ==================== الإضافة الثانية: وظيفة لاختيار التاريخ الجديد ====================
          ...
          title: Column(
            children: [
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  // ==================== التعديل: إضافة واجهة عرض التاريخين ====================
                  Row(
                    children: [
                      GestureDetector(
                        onTap: () => _selectDate(context),
                        child: Text(
                          'من: ${selectedDate.toLocal().toString().split(' ')[0]}',
                      ...
                      if(widget.mainkey==2)
                      GestureDetector(
                        onTap: () => _selectToDate(context), // استدعاء الوظيفة الجديدة
                        child: Text(
                          'إلى: ${selectedToDate.toLocal().toString().split(' ')[0]}', // عرض التاريخ الجديد
                          style: TextStyle(fontSize: 14),
                        ),
                      ),
                    ],
                  ...
                  Row(
                    children: [
                      Directionality(
                        ...
                        child: Row(
                          children: selectedAreas.map((areaCode) {
                            bool isLast = selectedAreas.last == areaCode;
                            return Container(
                              margin: const EdgeInsets.only(left: 4),
                          ...
                          onTap: () {
                            Navigator.push(
                              ...
                              MaterialPageRoute(
                                builder: (context) => DataTableScreen(
                                  checkdate: checkdate,
                                  selectedDate_from: selectedDate,
                                  selectedDate_to: selectedToDate,
                                  processName: process['group_process_name'],
                                ),
                              ),
                            ).then((_) {
